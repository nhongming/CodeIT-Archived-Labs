...
## Alternative Approach (Using InOrder Traversal Recursion)

def printPlayersInRange(min, max, root):
    # TO IMPLEMENT
    # base case where Treenode is empty
    if root == None:
        return
    # Since the players BST is already sorted, recurse for left
    # subtree first. If node rating is greater than min rating search parameter,
    # then only we can get matching ratings in left subtree
    if min < root.key:
        printPlayersInRange(min, max, root.left)
    # If rating lies within range, then print root's data
    if min <= root.key and root.key <= max:
        matched_rating = root.key
        matched_players_arr = root.value
        print('overall: {}'.format(matched_rating))
        print(matched_players_arr)
        print()
    
    # recursively call the right subtree
    printPlayersInRange(min, max, root.right)
...
def main():
    ...
    root = players.root
    printPlayersInRange(min, max, root)
